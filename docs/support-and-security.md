...
Upload-candidate, package-dry-run, source No-Go, license, or provenance questions can use public issues only when the example is synthetic or already public and does not include private source archives, unreleased source code, file inventories from private projects, raw scanner findings, personal data, credentials, account data, transfer URLs, or resource contents that should not be public.
...
Do not post secrets, exploit details, private account data, personal data, moderation evidence, uploader tokens, upload session URLs, transfer URLs, raw parser inputs, raw parser outputs, raw imported content, raw scanner findings, private source inventories, live resolver output, tool responses, or unsafe resource contents in public issues. Use the private or platform-owned route for sensitive reports.
...
- Upload-candidate, package-dry-run, source No-Go, license, or provenance wording changes require no-execution, report-only, and no-overclaim review.
