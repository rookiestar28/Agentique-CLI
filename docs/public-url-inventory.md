...
Current status: source repository, schema, documentation, package registry pages, action usage reference, badge/readback documentation, and existing `agentique.io` public links are approved for advertising after publication and smoke testing. The uploader package page is approved after owner-approved publication, registry readback, and clean install smoke. Source-branch catalog/download package changes and live availability are not newly advertised until their scoped release and live evidence gates pass.
...
- `agentique.io` read-only public readback endpoint: approved for existing inventory; not a current catalog/download live availability claim for source-branch changes.
