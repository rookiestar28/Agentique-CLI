...
Graph/block source changes are local preparation surfaces in this source revision. Package publication and runtime claims remain No-Go until hosted release evidence, package publication, registry readback, and clean registry install smoke exist for a future package version containing these changes.
