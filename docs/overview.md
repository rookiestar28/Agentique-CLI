...
These docs help external creators prepare public resources before entering the `agentique.io` upload flow, validate package structure with public tooling, inspect non-static lane descriptors, review source-only uploader behavior, and display resource status through public readback after platform publication.

The docs explain resource manifests, skill packages, workflow templates, non-static lane examples, distribution mode choices, source uploader boundaries, scan readback, and support routing.
...
5. Enter the platform-owned upload flow on `agentique.io`, or review a source-only uploader session when explicitly configured with platform API access.
6. Wait for platform scan, review, consent, moderation, publication, and readback.

## Public Boundaries

Companion docs and source tools do not publish, edit, delete, moderate, approve, or certify resources. The source uploader preview is review-only and is not advertised as an npm package or live upload channel. Public issue threads are for docs and tooling questions, not unsafe reports or private account matters.
