...
`agentique-validator` is a no-execution checker that validates public manifests, package inventory, path safety, registry trust metadata, and upload-prep metadata without uploading, publishing, installing dependencies, or executing submitted code.
...
External intake output is advisory review evidence. Registry trust findings are local preparation findings. Neither output is publication approval, safety assurance, moderation status, or legal review.
