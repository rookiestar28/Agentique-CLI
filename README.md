...
This repository revision also includes parser import dry-runs, variant dry-runs, agent-native dry-runs, public catalog reads, download-metadata reads, and direct artifact byte download for local review. The full companion package set is published at `0.2.1` after hosted CI, GitHub Actions registry provenance, registry readback, and registry install smoke. Owner-approved disposable byte-transfer smoke passed for a public catalog resource.
